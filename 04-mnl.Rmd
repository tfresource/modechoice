...
thegoods2 <- data.frame(
 "Alternative"= c("Drive Alone","Shared Ride","Transit"),
 "Expression" = c("1.1 + 0.0008 * 50^7 - 0.02 * 30^8","0.8 + 0.006 * 50 - 0.02 * 35","0.0 + 0.000 * 50 - 0.02 * 50"),
 "Value" = c(0.90,0.40,-1.00),
 "Exponent" = c(2.460,1.492,0.368),
 "Probability" = c(0.569,0.345,0.085))

kbl(thegoods2, align = "c", booktabs = T, caption = "Utility and Probability calculation with TRansit as Base Alternative") %>% 
kable_classic() %>% 
  add_header_above(c(" "=1, "Utility" = 2, " " = 2), align = "center")
```
...
thegoods3 <- data.frame(
 "Alternative"= c("Drive Alone","Shared Ride","Transit"),
 "Expression" = c("0.0 + 0.000 * 50 - 0.02 * 30","-0.3 + 0.002 * 50 - 0.02 * 35","-1.1 - 0.008 * 50 - 0.02 * 50"),
 "Value" = c(-0.60,-1.10,-2.50),
 "Exponent" = c(0.549,0.333,0.082),
 "Probability" = c(0.569,0.345,0.085))

kbl(thegoods3, align = "c", booktabs = T, caption = "Utility and Probability calculation with Drive Alone
    as Base Alternative") %>% 
kable_classic() %>% 
  add_header_above(c(" "=1, "Utility" = 2, " " = 2), align = "center")
