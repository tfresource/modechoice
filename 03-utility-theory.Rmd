...
Travel time and travel cost are generic; that is, they apply to all alternatives; frequency is specific to transit only. The parameters,  , are identical for all the alternatives to which they apply.  This implies that the utility value of travel time and travel cost are identical across alternatives.  The possibility that travel time may be more onerous on Transit than by Drive Alone or Shared Ride could be tested by reformulating the above models to:
...
|   where   $\beta_{i0}$  is the modal bias constant for mode $i$ ($i$=DA,SR,TR),
...
where $V()$ represents the deterministics components of the utility for the alternatives, and 
