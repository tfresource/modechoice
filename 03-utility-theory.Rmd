...
```{r detchoice, fig.cap="Illustration of Deterministic Choice"} 
...
\begin{equation}
V(X_i) = γ_1 ~ ' ~ X_{i1} + γ_2 ~ ' ~ X_{i2} + L + γ_k ~ ' ~ X_{ik}
(\#eq:threefour)
\end{equation}

where

  - $γ_k$ is the parameter which defines the direction and importance of the  
...
That is, two distinct parameters would be estimated for travel time; one for travel time by DA and SR,  , and the other for travel time by TR, $g_{11}$ .  These parameters could be compared to determine if the differences are statistically significant or large enough to be important.
