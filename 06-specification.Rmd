...
```{r Table-6-2, echo = F}
#not sure if this table is relevant given that fact that we arent' restricting model 1 or 2. Also, why are we
#restricting model 3, but not the others? I haven't figured out a way to determine the rejection levels for the models.
# I'm not entirely sure this table is useful (now or in the past) except for the fact that it displays the logliklihood ratio # test
tibble(
"Test" = c("Loglikelihood Ratio Test", "Degrees of Freedom", "Rejection Significance Level"),
"Base Model vs Model 1W" = c(-2*(logLik(model_1w)-logLik(model_base)), 1, "p.value(model_1w)"),
"Base Model vs Model 2W" = c(-2*(logLik(model_2w)-logLik(model_base)), 2, "p.value(model_2w)"),
"Base Model vs Model 3W" = c(-2*(logLik(model_3w)-logLik(model_base)), 2, "p.value(model_3w)"),
"Model 1W vs Model 2W" = c(-2*(logLik(model_2w)-logLik(model_1w)), 1, "p.value(model_2w)"),
"Model 1W vs model 3W" = c(-2*(logLik(model_2w)-logLik(model_3w)), 1, modelsummary(model_3w, statistic = 'p.value'))
)%>%
   kbl(align = 'c', caption = "Likelihood Ratio Tests between Models in Table 6-1 (Likelihood Ratio Statistic, Degrees of Freedom, and Rejection Significance Level") %>%
   kable_styling()
```
...
tibble(distance = c(5, 10, 20)) %>%
  rowwise() %>%
  mutate(
  "mot_ovtt" = model7_vot(model_7w, "mot_ovttbydist", "cost", distance),
  "mot_ivtt" = model7_vot(model_7w, "mot_tvtt", "cost", distance),
  "mot_tvtt" = mot_ovtt + mot_ivtt,
  "nm_tvtt" = model7_vot(model_7w, "nm_tvtt", "cost", distance),
) %>%
...
model_15w <- mlogit(chosen ~ mot_tvtt + nm_tvtt + I(mot_ovtt/dist) + cost | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext)
...
tibble(
      "Measure" = c("Value of In-Vehicle Time","Value of Out-of-Vehilce Time (10 mile trip)", "Value of Out-of-Vehilce Time (20 mile trip)"),
       "Model 15W" = c(
       ...
       "Model 16W (Wage Rate = $21.20)" = c(
         paste("0.47 x Wage Rate ($", round(0.47*21.20,2), "/hr)"), 
         paste("0.77 x Wage Rate ($", round(0.77*21.20,2), "/hr)"), 
         paste("0.62 x Wage Rate ($", round(0.62*21.20,2), "/hr)"))
      ) %>%
