...
sf_mlogit_tripcontext <- sf_mlogit %>%
  ...
  mutate(autoperhh = numveh / hhowndum) %>%
  mutate(autoperad = numveh / numadlt)

model_7w <- mlogit(chosen ~ mot_tvtt + nm_tvtt + mot_ovtt + cost | hhinc, data = sf_mlogit_tripcontext)
model_10w <- mlogit(chosen ~ mot_tvtt + nm_tvtt + mot_ovtt + cost | hhinc + autoperhh, data = sf_mlogit_tripcontext)
model_11w <- mlogit(chosen ~ mot_tvtt + nm_tvtt + mot_ovtt + cost | hhinc + vehbywrk, data = sf_mlogit_tripcontext)
model_12w <- mlogit(chosen ~ mot_tvtt + nm_tvtt + mot_ovtt + cost | hhinc + autoperad, data = sf_mlogit_tripcontext)
