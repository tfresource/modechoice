...
Area was reported in Table 5-2 \@ref(basic-estimation-table) in CHAPTER 5 \@ref(mnl-estimation.Rmd). The refinements we consider include:
...
model_3w <- mlogit(chosen ~ tvtt + cost | hhinc, data = sf_mlogit, constPar = c('hhinc:2' = 0, 'hhinc:3' = 0))
...
\begin{equation}
Value of motorized IVTT (\$/hour) = \frac{\beta_{motorized ivtt (1/min.)}}{\beta_{cost (1/cents)}} \times \frac{60 min./hour}{100 cents/\$} 
(\#eq:valueofivtt)
\end{equation}
...
```{r Table 6-4 Vot models 1w 5w and 6w, echo = FALSE}
...
tibble(
  ...
  "Model 1W" = c(VOTsimple(model_1w, "tvtt", "cost"), VOTsimple(model_1w, "tvtt", "cost"), VOTsimple(model_1w, "tvtt", "cost")),
  "Model 5W" = c(VOTsimple(model_14w, "mot_tvtt", "cost"), VOTdistance(model_14w, "mot_tvtt", "mot_ovttbydist", 20, "cost"), VOTsimple(model_14w, "nm_tvtt", "cost")),
  "Model 6W" = c(VOTsimple(model_15w, "mot_tvtt", "cost"), VOTdistance(model_15w, "mot_tvtt", "mot_ovttbydist", 20, "cost"), VOTsimple(model_15w, "nm_tvtt", "cost"))
) %>%
...
```{r Table 6-6 Model 7W Implied Values of Time as a Function of Trip Distance, echo = F}

"Value of Motorized Out-of-Vehicle Time"
"Value of Motorized In-Vehicle Time"
"Value of Non-Motorized Time"

```

```{r Table 6-7 Implied Values of Time in Models 6W, 8W, Model 9W, echo = F}
...
```{r Table 6-10 trip context VoT from models, echo = F}
#These functions are based off the equations given in the text above Table 6-6
VOTsimple <- function(model, timevar, costvar) {
  coef(model)[timevar]*0.6/coef(model)[costvar]
}
...
tibble(
  ...
  "Model 13W" = c(VOTsimple(model_13w, "mot_tvtt", "cost"), VOTdistance(model_13w, "mot_tvtt", "mot_ovttbydist", 10, "cost"), VOTdistance(model_13w, "mot_tvtt", "mot_ovttbydist", 20, "cost"), VOTsimple(model_13w, "nm_tvtt", "cost")),
  "Model 14W" = c(VOTsimple(model_14w, "mot_tvtt", "cost"), VOTdistance(model_14w, "mot_tvtt", "mot_ovttbydist", 10, "cost"), VOTdistance(model_14w, "mot_tvtt", "mot_ovttbydist", 20, "cost"), VOTsimple(model_14w, "nm_tvtt", "cost")),
  "Model 15W" = c(VOTsimple(model_15w, "mot_tvtt", "cost"), VOTdistance(model_15w, "mot_tvtt", "mot_ovttbydist", 10, "cost"), VOTdistance(model_15w, "mot_tvtt", "mot_ovttbydist", 20, "cost"), VOTsimple(model_15w, "nm_tvtt", "cost"))
) %>%
  kbl(align = 'c', caption = "Implied Values of Time in Models 13W, 14W, and 15W") %>%
  kable_styling()
```
...
#Create model 16w
model_16w <- mlogit(chosen ~ mot_tvtt + nm_tvtt + mot_ovttbydist + costbyinc | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext)
...
``` {r impliedVOT15-16W}
#functions below are defined earlier :)
#VOTsimple <- function(model, timevar, costvar) {
#  coef(model)[timevar]*0.6/coef(model)[costvar]
#}
#VOTdistance <- function(model, timevar, timedistvar, dist_value, costvar) {
...
#table 6-12
tibble ( c("Value of In-Vehicle Time", "Value of Out-of-Vehicle Time (10 mile trip)", "Value of Out-of-Vehicle Time (20 mile trip)"), "Model 15W" = c(VOTsimple(model_15w, "mot_tvtt", "cost"), VOTdistance(model_15w, "mot_tvtt", "mot_ovttbydist", 10, "cost"), VOTdistance(model_15w, "mot_tvtt", "mot_ovttbydist", 20, "cost")), "Model 16W" = c(0.47*21.20, 0.77*21.20, 0.62*21.20) )
...
# Model 17 W
model_17w <- mlogit(chosen ~ I(cost/hhinc) + mot_tvtt + nm_tvtt + I(mot_ovtt/dist) | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext, constPar = c("hhinc:Share ride 2" = 0, "hhinc:Share ride 3++" = 0))
...
model_17w_lowcars <- mlogit(chosen ~ I(cost/hhinc) + mot_tvtt + nm_tvtt + I(mot_ovtt/dist) | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext, constPar = c("hhinc:Share ride 2" = 0, "hhinc:Share ride 3++" = 0) %>% filter(numveh <= 1))
model_17w_highcars <- mlogit(chosen ~ I(cost/hhinc) + mot_tvtt + nm_tvtt + I(mot_ovtt/dist) | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext, constPar = c("hhinc:Share ride 2" = 0, "hhinc:Share ride 3++" = 0) %>% filter(numveh >= 2))
...
# uses Model 17W as the basis
model_17w_males <- mlogit(chosen ~ I(cost/hhinc) + mot_tvtt + nm_tvtt + I(mot_ovtt/dist) | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext, constPar = c("hhinc:Share ride 2" = 0, "hhinc:Share ride 3++" = 0) %>% filter(femdum == 1))
model_17w_females <- mlogit(chosen ~ I(cost/hhinc) + mot_tvtt + nm_tvtt + I(mot_ovtt/dist) | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext, constPar = c("hhinc:Share ride 2" = 0, "hhinc:Share ride 3++" = 0) %>% filter(femdum == 0))
# table 6-15
list(
  "Pooled Model" = model_17w,
