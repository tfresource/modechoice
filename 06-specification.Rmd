...
#Estimates are close but slightly off. I'm not sure how the variables might be off
#I added both columns of CBD workers together after neither gave the right answer.
#I'm not sure how to get travel time estimates for motorized/non-motorized modes
...
model_11w <- mlogit(chosen ~ tvtt + cost | hhinc + vehbywrk, data = sf_mlogit_tripcontext)
model_13w <- mlogit(chosen ~ tvtt + cost | hhinc + vehbywrk + cbddumall, data = sf_mlogit_tripcontext)
model_14w <- mlogit(chosen ~ tvtt + cost | hhinc + vehbywrk + wkempden, data = sf_mlogit_tripcontext)
model_15w <- mlogit(chosen ~ tvtt + cost | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext)
...
#I need to make the models above match the values in the text and update the values here
...
#new column for TT motorized only
morecolumns <- sf_mlogit_tripcontext
morecolumns$tvttMotor <- morecolumns$tvtt
morecolumns$tvttMotor[morecolumns$altname == "Bike"] <- 0
morecolumns$tvttMotor[morecolumns$altname == "Walk"] <- 0

#new column for TT non-motorized only
morecolumns$tvttNM <- 0
morecolumns$tvttNM[morecolumns$altname == "Bike"] <- morecolumns$tvtt
morecolumns$tvttNM[morecolumns$altname == "Walk"] <- morecolumns$tvtt

#new column for OVTT non-motorized only
morecolumns$ovttMotor <- morecolumns$ovtt
morecolumns$ovttMotor[morecolumns$altname == "Bike"] <- 0
morecolumns$ovttMotor[morecolumns$altname == "Walk"] <- 0

#the parameters in this model are not quite correct
#need to restrict tvtt to only non-motorized modes
model_16w <- mlogit(chosen ~ tvtt + I(ovtt + ivtt) + I(ovtt/dist) + I(cost/hhinc) | hhinc + vehbywrk + cbddumall + wkempden, data = sf_mlogit_tripcontext)
...
Thus, the null hypothesis is that β1 = β2 = ... = βs = ... = βS , where βs , is the
...
Substituting the log-likelihood for the pooled model for lR and the sum of market segment
