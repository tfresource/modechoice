...
#Here, I've tried to index the SF data, and either get errors that the indexes I use 
#don't create defined data (if I try to include more than one indexing factor), or I 
#get errors when trying to create choice models such as "labelled" errors or that I 
#can't create a vector that is 18Gb large. #I'm going to elect to leave this either
#until I'm able to get help figuring 
#this out, or until someone else that has more coding skills than I have does it.

# Convert into mlogit dfidx data frame
sf_mlogit <- dfidx(sf_work_mode, idx = list(c("perid")))

# write to data/ folder for future use
write_rds(sf_mlogit, "data-raw/Work Trips/sfwork_logit.rds")
```

```{r figure5-1, eval = F}
#Trying to make a choice model similar to the one we used for the fish data. 
#Unable to get this code to run successfully. If I can't get the data sets 
#indexed right, I cant easily recreate the tables shown in this figure. 
#I can just do it by hand, but that defeats the purpose of this exercise. 
#I'm going to elect to leave this either until I'm able to get help figuring 
#this out, or until someone else that has more coding skills than I have does it.

sf_base <- mlogit(altnum ~ cost + tvtt | wkempden, data = sf_mlogit)
...
The statistic used for testing the null hypothesis that a parameter $\hat{\beta_{k}}$ is equal to some hypothesized value, $\hat{\beta_{k}}$, is the asymptotic t-statistic, which takes the following form:   

\begin{equation}
$\displaystyle t-statistic = \frac{\hat{\beta_{k}}-\beta_{k}^{*}}{S_{k}}$
(\#eq:asymptotic t-statistic)
\end{equation}
