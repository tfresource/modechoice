...
incomebase_model <- mlogit(chosen ~ tvtt | hhinc, data = sf_mlogit_basemodels, constPar = c("tvtt" = 0))
...
thegoods1 <- tibble(
 "Variables"= c("Log-Likelihood of Unrestricted Model $(LL_U)$","Log-Likelihood of Restricted Model $(LL_R)$","Test Statistics [-2$(LL_R-LL_U)$]","Number of Restrictions","Critical Chi-Squared Value at 99.9% Confidence","Rejection Confidence","Rejection Significance"),
 "Test for Hypothesis $H_{0,c}$" = c(-3626.186,-4123.615,994.858,2,13.82, ">>99.9%", 0.000),
 "Test for Hypothesis $H_{0,d}$" = c(-3626.186, -3637.579, 22.786, 5, 20.51, ">>99.9%",0.000))
tibble(
  llu =c( -3636.186, 0),
  llr = c()
) %>%
  mutate(lrtests = -2 * (llr - llu))
...
base_model <- mlogit(chosen ~  tvtt | hhinc, data = sf_mlogit_costinc)
income_model <- mlogit(chosen ~ tvtt + costbyinc, data = sf_mlogit_costinc, )
lnincome_model <- mlogit(chosen ~ tvtt + costbylninc, data = sf_mlogit_costinc)
