...
kbl(head(trip_alt, n = 10), caption= "Data Layout Type II: Trip-Alternative Format") %>%
  kable_styling()
```
...
trip_format <- sf_work_mode  %>%
  ...
  group_by(hhid, perid) %>%
  mutate( chosen = max(chosen * altnum), altname = str_c(altname, " - Travel Time")) %>%
  select(-altnum) %>%
...
vot <- function(model, triptime){
  coef(model)["lntraveltime"] / (coef(model)["cost"]*triptime)
}
vot5 <- vot(lntravaltime_model, 5)
vot15 <- vot(lntravaltime_model, 15)
vot30 <- vot(lntravaltime_model, 30)
vot60 <- vot(lntravaltime_model, 60)
vot90 <- vot(lntravaltime_model, 90)
vot120 <- vot(lntravaltime_model, 120)

votlnTime1 <- tibble(
  TTime = c(5, 15, 30, 60, 90, 120),
  ValueCentsMin = c(vot5, vot15, vot30, vot60, vot90, vot120)
)
votlnTime1 %>%
  select(TTime, ValueCentsMin) %>%
  mutate(ValueDollarsHr = ValueCentsMin * 0.6) %>%
  kbl(align = "c", booktabs = T, format = "html", digits = 2, 
