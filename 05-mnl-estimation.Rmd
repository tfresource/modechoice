...
#display the table =================================
kbl(statsJtoWork, align = "c", booktabs = T, caption = "Sample Statistics for Bay Area Journey-to-Work Modal Data") %>% 
  kable_classic() %>%
  row_spec(0, bold = T)
...
tibble(
  confid = c(.9, .95, .99, .995, .999)) %>%
  mutate(tval=qnorm((1-confid)/2, lower.tail = FALSE)) %>%
  knitr::kable( caption = "Critical t-Values for Selected Confidence Levels and Large Samples",
...
ggplot(data.frame(x = c(-4,4))) +
  ...
  stat_function(
    ...
    aes(x),
    geom = "area", color = NA, fill = "grey50", alpha = 1) +
  # Shade above 1.645
   stat_function(
    fun = function(x) ifelse(x >= 2, dt(x, df = 29), NA),
    aes(x),
    geom = "area", color = NA, fill = "grey40", alpha = 1) +
   # Shade below -1.645
  stat_function(
    fun = function(x) ifelse(x <= -1.645, dt(x, df = 29), NA),
    aes(x),
...
#trying to get modelsummary::modelsummary to work, but haven't figured it out yet
...
ggplot(data.frame(x = c(0, 30)), aes(x = x)) +
       ...
       stat_function(fun = dchisq, args = list(df = 15), color = "red") +
       labs(x = "Test Values",
            y = "Chi-Squared")
...
ggplot(data.frame(x = c(0, 20)), aes(x = x)) +
       stat_function(fun = dchisq, args = list(df = 5)) +
       labs(title = "Chi-Squared Distribution for 5 Degrees of Freedom Showing 90% and 95% confidence Thresholds",
            x = "Test Values",
            y = "PDF")
```
...
```{r table5-6, echo = F}
#I think I need to do a log likelihood test, but I don't know how to start or what data to use for models.

tab56df <- data.frame(
 "Variables."= c("Log-Likelihood of Unrestricted Model ($LL_U$)", "Log-Likelihood of Restricted Model ($LL_R$)", "Test Statistics [-2($LL_R-LL_U$)]", "Number of Restrictions", "Critical Chi-Squared Value at 99.9% Confidence", "Rejection Confidence", "Rejection Significance"),
 "Test for Hypothesis H_{0,a}" = c("-3626.186", "-7309.301", "7366.830", "12", "32.91", "99.9%", "0.001"),
 "Test for Hypothesis H_{0,b}" = c("-3626.186", "-4132.916", "1013.460", "7", "24.32", "99.9%", "0.001"))

kbl(tab56df, align = "c", booktabs = T, caption = "Likelihood Ratio Test for Hypothesis $H_{0,a}$ and $H_{0,b}$") %>% 
kable_classic()
```
...
```{r table 5-7, echo = F}
#I have no idea what data to use, or how to format it or model it to create this table.
...
thegoods1 <- tibble(
 "Variables"= c("Log-Likelihood of Unrestricted Model $(LL_U)$","Log-Likelihood of Restricted Model $(LL_R)$","Test Statistics [-2$(LL_R-LL_U)$]","Number of Restrictions","Critical Chi-Squared Value at 99.9% Confidence","Rejection Confidence","Rejection Significance"),
 "Test for Hypothesis $H_{0,c}$" = c(-3626.186,-4123615,994.858,2,13.82, ">>99.9%", 0.000),
 "Test for Hypothesis $H_{0,d}$" = c(-3626.186, -3637.579, 22.786, 5, 20.51, ">>99.9%",0.000))

kable(thegoods1, align = c("l", "c" ,"c"), booktabs = T, caption = "Likelihood Ratio Test for Hypothesis $H_{0,c}$ and $H_{0,d}$")
```
...
```{r basic-estimation}
costbyinc <- sf_mlogit$cost / sf_mlogit$hhinc
sf_mlogit <- cbind(sf_mlogit, new_col = costbyinc) 

costbylninc <- sf_mlogit$cost / log(sf_mlogit$hhinc)
sf_mlogit <- cbind(sf_mlogit, new_col = costbylninc)

base_model <- mlogit(cost ~  tvtt + altnum | hhinc, data = sf_mlogit)
income_model <- mlogit(cost ~ tvtt + altnum | costbyinc, data = sf_mlogit)
lnincome_model <- mlogit(cost ~ tvtt + altnum | cost/log(hhinc), data = sf_mlogit)
...
```{r table5-12, echo = F}
#this code will also produce Figure 5-7


logTime_model <- mlogit(chosen ~ log(tvtt) + cost | hhinc, data = sf_mlogit)
summary(logTime_model)

# time value of money is usually calculated by dividing Beta_cost / Beta_tvtt
#I'm not sure how changing the trip time changes the time value of money
