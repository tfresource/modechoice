...
```{r table5-8}
tibble(
  llu = rep(base_model$logLik, 2),
  llr = c(incomebase_model$logLik, timecostbase_model$logLik)
) %>%
  mutate(
    test_stat = -2 * (llr - llu),
    df = c(2, 5),
    chisqcrit = qchisq(0.999, df),
    pval = pchisq(test_stat, df, lower.tail = FALSE)
  ) %>%
  kbl(caption = "Likelihood Ratio Test for Hypothesis $H_{0,c}$ and $H_{0,d}$") %>%
  kable_styling()
```
